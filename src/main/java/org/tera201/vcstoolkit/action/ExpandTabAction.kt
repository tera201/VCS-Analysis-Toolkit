...
class ExpandTabAction(private val actionManager: ActionManager) :
    DumbAwareAction(Supplier { "Expand tab" }, AllIcons.General.ExpandComponentHover) {
    override fun actionPerformed(event: AnActionEvent) {
        ...
        event.project?.let {
            val jfxPanel =
                (content as JComponent).components.stream().filter { it is JFXPanel }.findAny().get() as JFXPanel
